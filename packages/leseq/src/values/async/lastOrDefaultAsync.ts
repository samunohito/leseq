 ...
 * @category Values
 */
export const lastOrDefaultAsync = <T>(predicate: (arg: T, index: number) => Promise<boolean> = () => Promise.resolve(true), defaultValue: T | undefined = undefined) =>
  async function lastOrDefaultAsync(seq: AsyncSeq<T>): Promise<T | undefined> {
